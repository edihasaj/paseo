...
export async function startPackagedWebDaemon(input: {
  ...
  const stroll = path.resolve(__dirname, "../../../../../node_modules/.bin/stroll");
  ...
  try {
    ...
    return {
      ...
      close: async () => {
        await execFileAsync(stroll, ["daemon", "stop", "--home", home], { env }).catch(
          () => undefined,
        );
    ...
    await execFileAsync(stroll, ["daemon", "stop", "--home", home], { env }).catch(() => undefined);
    await rm(home, { recursive: true, force: true });
    throw error;
  }
}
