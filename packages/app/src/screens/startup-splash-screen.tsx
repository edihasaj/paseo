...
function WebLogoShimmer({ color }: { color: string }) {
  ...
  const shimmerStyle = useMemo(
    () => ({
      width: LOGO_SIZE,
      height: LOGO_SIZE,
      WebkitMaskImage: `url("data:image/svg+xml,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='${LOGO_SIZE}' height='${LOGO_SIZE}' viewBox='0 0 700 700'><path fill='none' stroke='black' stroke-width='67' stroke-linecap='round' stroke-linejoin='round' d='M 350 566 C 169.5 507.2, 169.5 404.7, 350 350 C 530.5 295.3, 530.5 192.8, 350 134'/></svg>`)}")`,
  ...
  const maskElement = useMemo(
    () => (
      <View style={styles.shimmerMask}>
        <StrollLogo size={LOGO_SIZE} color="#000000" />
      </View>
    ),
    [],
  );
    ...
    <MaskedView style={trackStyle} maskElement={maskElement}>
      <View style={trackStyle}>
        <View style={styles.nativeShimmerBase}>
          <StrollLogo size={LOGO_SIZE} color={color} />
        </View>
