...
export function releaseBase(version: string) {
  return `https://github.com/edihasaj/paseo/releases/download/v${version}`;
}
...
export function downloadUrls(release: ReleaseAssetInfo) {
  ...
  return {
    macAppleSilicon: `${base}/Stroll-${version}-arm64.dmg`,
    macIntel: `${base}/Stroll-${version}-x64.dmg`,
    linuxAppImage: `${base}/${linuxAppImageAsset}`,
    linuxDeb: `${base}/Stroll-${version}-amd64.deb`,
    linuxRpm: `${base}/Stroll-${version}-x86_64.rpm`,
    windowsExeX64: `${base}/${windowsX64Asset ?? `Stroll-Setup-${version}.exe`}`,
