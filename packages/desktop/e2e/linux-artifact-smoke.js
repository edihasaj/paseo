...
async function main() {
  ...
  try {
    ...
    const helper = fs.statSync("/opt/Stroll/chrome-sandbox");
    ...
    await smokePackagedDesktopApp({ appPath: "/opt/Stroll", expectedSandbox: true });
    ...
    const desktopEntry = fs.readFileSync(path.join(appDir, "Stroll.desktop"), "utf8");
    ...
    const appPath = fs.existsSync(path.join(tarDir, "Stroll"))
      ? tarDir
