...
function createFakeMacBundle(options: { includeHelper: boolean }): {
  ...
  const root = mkdtempSync(join(tmpdir(), "stroll-cli-shim-test-"));
  const appPath = join(root, "Stroll.app");
  const contentsPath = join(appPath, "Contents");
  const resourcesPath = join(contentsPath, "Resources");
  const shimPath = join(resourcesPath, "bin", "stroll");
  const mainPath = join(contentsPath, "MacOS", "Stroll");
  ...
  copyFileSync(join(packageRoot, "bin", "stroll"), shimPath);
...
describe("desktop packaging", () => {
  ...
  it("registers Stroll agent links with the operating system", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("name: Stroll agent link");
  ...
  it("fails packaged macOS CLI startup when Helper is missing", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Bundled Stroll Helper executable not found");
      expect(result.stdout).not.toContain("main-executable");
    } finally {
      rmSync(bundle.root, { recursive: true, force: true });
    }
