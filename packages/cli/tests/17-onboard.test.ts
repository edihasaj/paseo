...
try {
  ...
  const onboard =
    await $`PASEO_HOME=${paseoHome} PASEO_LISTEN=127.0.0.1:${port} PASEO_PAIRING_QR=0 npx stroll`.nothrow();
  ...
  const status =
    await $`PASEO_HOME=${paseoHome} npx stroll daemon status --home ${paseoHome}`.nothrow();
  ...
  const enableRelay =
    await $`PASEO_HOME=${paseoHome} npx stroll daemon pair --home ${paseoHome} --relay`.nothrow();
  ...
  const noRelayOnboard =
    await $`PASEO_HOME=${paseoHome} PASEO_LISTEN=127.0.0.1:${port} npx stroll --no-relay`.nothrow();
  assert.strictEqual(
  ...
  await $`PASEO_HOME=${paseoHome} npx stroll daemon stop --home ${paseoHome} --force`.nothrow();
