...
try {
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
