...
try {
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} wait abc123`.nothrow();
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
