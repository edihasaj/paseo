...
try {
  ...
  // Test 3: worktree ls handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} worktree ls`.nothrow();
  ...
  // Test 4: worktree ls with --host flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} worktree ls --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: worktree archive requires name argument
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} worktree archive`.nothrow();
  ...
  // Test 7: worktree archive handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} worktree archive test-worktree`.nothrow();
    // Should fail because daemon not running
    assert.notStrictEqual(result.exitCode, 0, "should fail when daemon not running");
    const output = result.stdout + result.stderr;
    const hasError =
  ...
  // Test 8: worktree archive with name and --host flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} worktree archive test-worktree --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
    assert(!output.includes("unknown option"), "should accept --host flag");
    assert(!output.includes("error: option"), "should not have option parsing error");
  ...
  // Test 9: -q (quiet) flag is accepted with worktree ls
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} -q worktree ls`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: --json flag is accepted with worktree ls
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} worktree ls --json`.nothrow();
    const output = result.stdout + result.stderr;
