...
try {
  ...
  // Test 2: inspect requires id argument
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} inspect`.nothrow();
  ...
  // Test 3: inspect handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} inspect abc123`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted with inspect
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} -q inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: --json flag is accepted with inspect
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} inspect abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --format yaml flag is accepted with inspect
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx stroll --host localhost:${port} --format yaml inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
