...
test("cancelAgentRun settles instead of leaving a permanent running state when recovery fails", async () => {
  const fixture = await createControlledInterruptFixture({
  ...
  try {
    ...
    // Reload retires the unresponsive session before requesting its replacement, so only one
    // writer ever touches the persisted thread. When the replacement itself cannot be
    // established there is no live session left to resurrect; the agent is gone rather than
    // stuck reporting "running" forever.
    expect(fixture.manager.getAgent(fixture.agentId)).toBeNull();
  } finally {
    await fixture.cleanup();
  }
});
