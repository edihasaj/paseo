...
One rounded card — `borderRadius["2xl"]`, `border`, `background` surface, `theme.shadow.sm` — holds the input and its controls row, on the `MAX_CONTENT_WIDTH` rails (`packages/app/src/composer/input/input.tsx`). The input sits on top; the controls row sits below it, inside the same card, never a second bordered surface stacked underneath. Focus raises the border to `borderAccent` only — no glow, no shadow change. The card's min height fits two lines of `fontSize.content` plus the controls row; input padding is `spacing[4]` horizontal, `spacing[3]` top. Placeholder text is `foregroundMuted` — the app-wide floor (§14) applies here too. Attachment pills render in a tray inside the card, above the input, never a second bordered surface. Compact/mobile keeps the same card with tighter padding.
