...
One rounded card — `borderRadius.lg`, `border`, `background` surface, `theme.shadow.sm` — holds the input and its controls row, on the `MAX_CONTENT_WIDTH` rails (`packages/app/src/composer/input/input.tsx`). The input sits on top; the controls row sits below it, inside the same card, never a second bordered surface stacked underneath.
...
- **Right.** An inline `LoadingSpinner` while the agent is running, the model combobox trigger, the thinking/effort control (`Zap` glyph), the dictation mic, then the send button.
...
The send button is a filled `accent` circle with an up-arrow; while the agent runs it becomes the same circle filled `destructive` with a stop-square glyph (`ComposerCancelButton` in `composer/index.tsx`). Both are the same button changing state, not two components.
